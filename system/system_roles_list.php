<?
	...
	if ($action == "update" & is_numeric($system_group_role_id)) {
		...
		add_system_records("system","system_roles","$system_group_role_id","","Update","");
		...
		add_system_group_role($system_group_role_update);
		add_system_records("system","system_roles","$system_group_role_id","","Insert","");
		foreach($system_group_role_write_access as $item) { 
			if ($item> 0) {
			add_system_authorization_group_role_join($system_group_role_id,$item);
			}
	...
	if ($action == "disable") {
		...
		add_system_records("system","system_roles","$system_group_role_id","","Disable","");
	}

	if ($action == "csv") {
		export_system_group_role_csv();
		add_system_records("system","system_roles","$system_group_role_id","","Export","");
	}
...
echo "					<td>";
	foreach($pre_selected_system_group_role_read_access as $pre_selected_auth_item) {
		...
		echo "- $auth_name[system_authorization_section_cute_name] / $auth_name[system_authorization_subsection_cute_name] - ($auth_name[system_authorization_action_cute_name])<br>";
	}
echo "					</td>";
echo "				</tr>";
	}
