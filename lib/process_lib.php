...
function update_process($process_data, $process_id) {
	$sql = "UPDATE process_tbl
		...
		process_rto=\"$process_data[process_rto]\"
		WHERE
		...
		if ($pre_selected_items) { 
			...
			foreach($pre_selected_items as $preselected) {
				...
				if ($process_item[process_id] == $preselected) {
					echo "<option selected=\"selected\">$process_item[process_name]</option>\n";
					$match = 1;
				} 
			}
			...
			# MUST EDIT
			if (!$match) { 
				echo "<option>$process_item[process_name]</option>\n"; 
			}
...
function export_process_csv() {
	...
	fwrite($handler, "process_id,process_name,process_description,process_disabled\n");
	foreach($result as $line) {
		fwrite($handler,"$line[process_id],$line[process_name],$line[process_descripion],$line[process_disabled]\n");
	}
