...
function list_drop_menu_asset_classification($pre_selected_items='', $order_clause='', $asset_classification_type) {
	...
	# MUST EDIT
	$sql = "SELECT * FROM asset_classification_tbl WHERE asset_classification_type = \"$asset_classification_type\"".$order_clause;
	echo "PUTA: $sql";
	$results = runQuery($sql);
...
function export_asset_classification_csv() {
	...
	foreach($result as $line) {
		fwrite($handler,"$line[asset_classification_id],$line[asset_classification_name],$line[asset_classification_descripion],$line[asset_classification_type], $line[asset_classification_value],$line[asset_classification_disabled]\n");
	}
