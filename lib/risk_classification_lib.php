...
function list_drop_menu_risk_classification($pre_selected_items='', $order_clause='', $risk_classification_type) {
	...
	# MUST EDIT
	$sql = "SELECT * FROM risk_classification_tbl WHERE risk_classification_type = \"$risk_classification_type\"".$order_clause;
	echo "PUTA: $sql";
	$results = runQuery($sql);
...
function export_risk_classification_csv() {
	...
	foreach($result as $line) {
		fwrite($handler,"$line[risk_classification_id],$line[risk_classification_name],$line[risk_classification_descripion],$line[risk_classification_type], $line[risk_classification_value],$line[risk_classification_disabled]\n");
	}
